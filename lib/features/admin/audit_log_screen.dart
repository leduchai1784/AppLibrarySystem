...
class _AuditLogScreenState extends State<AuditLogScreen> {
  ...
  ({IconData icon, Color color, String label}) _actionMeta(
    ThemeData theme,
    String actionRaw,
  ) {
    ...
    switch (a) {
      ...
      default:
        return (
          ...
          label: actionRaw.isEmpty ? '—' : actionRaw,
        );
    }
  }
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return Center(
              child: Text(t.auditLogLoadError('${snapshot.error}')),
            );
          }
          ...
          final filtered = docs.where((doc) {
            ...
            if (_filterAction != _all &&
                action.trim().toLowerCase() != _filterAction) {
              return false;
            }
            if (q.isEmpty) return true;
            final actor = '${d['actorEmail'] ?? d['actorUid'] ?? ''}';
            final detail = '${d['detail'] ?? ''}';
            final target = '${d['targetUserId'] ?? ''}';
            return foldedContains(actor, q) ||
                foldedContains(detail, q) ||
                foldedContains(target, q) ||
                foldedContains(action, q);
          }).toList();
