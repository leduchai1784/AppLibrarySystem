  ...
  Future<void> _confirmBulkDelete(AppLocalizations t) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(t.genresBulkDeleteConfirm(n)),
        actions: [
          TextButton(
    ...
    try {
      ...
      for (var i = 0; i < list.length; i += chunk) {
        ...
        for (var j = i; j < end; j++) {
          batch.delete(db.collection(GenreManageScreen._col).doc(list[j]));
        }
        await batch.commit();
      }
      if (!mounted) return;
      messenger.showSnackBar(
        SnackBar(content: Text(t.genresBulkDeleteResult(n))),
      );
      _exitSelectMode();
    } catch (e) {
      if (mounted) {
        messenger.showSnackBar(
          SnackBar(content: Text(t.genresDeleteError('$e'))),
        );
      }
    } finally {
        ...
        Scaffold(
          ...
          body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            stream: FirebaseFirestore.instance
                .collection(GenreManageScreen._col)
                .snapshots(),
            builder: (context, snapshot) {
              ...
              if (snapshot.hasError) {
                return Center(
                  child: Text(t.genresLoadError('${snapshot.error}')),
                );
              }
              final docs =
  ...
  Future<void> _showAdd(BuildContext context) async {
    ...
    try {
              ...
              onPressed: () {
                ...
                if (n.isEmpty) {
                  ScaffoldMessenger.of(
                    ctx,
                  ).showSnackBar(SnackBar(content: Text(t.genresNameRequired)));
                  return;
                }
                Navigator.pop(ctx, n);
              },
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(t.genresAdded(name))));
      }
    } finally {
      c.dispose();
    }
  ...
  Future<void> _showEdit(
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(t.genresUpdated)));
      }
    } finally {
      c.dispose();
    }
  ...
  Future<void> _confirmDelete(
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(t.genresDeleted)));
      }
    } catch (e) {
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(t.genresDeleteError('$e'))));
      }
    }
  }
}
