...
class _UserManageScreenState extends State<UserManageScreen> {
  ...
  Future<void> _setRole(_UserItem u, String newRole) async {
    ...
    try {
      ...
      await AuditLogService.roleChanged(
        targetUserId: u.id,
        fromRole: u.role,
        toRole: r,
      );
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(t.userManageRoleChanged)));
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(t.userManageRoleError('$e'))));
      }
    }
  }
      ...
      return Scaffold(
        ...
        body: Center(
          child: Padding(
            ...
            child: Text(t.staffOnlyUserManage, textAlign: TextAlign.center),
          ),
        ),
      );
    }
                ...
                Row(
                  children: [
                    ...
                    Tooltip(
                      ...
                      child: IconButton(
                        onPressed: () =>
                            setState(() => _denseMode = !_denseMode),
                        icon: Icon(
                          _denseMode
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                if (q.isNotEmpty) {
                  users = users.where((u) {
                    return foldedContains(u.name, q) ||
                        foldedContains(u.email, q);
                  }).toList();
                }
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final displayName = u.name.trim().isNotEmpty
                        ? u.name.trim()
                        : t2.userNamePlaceholder;
                    final displayEmail = u.email.trim().isNotEmpty
                        ? u.email.trim()
                        : 'UID: ${u.id}';
                    final roleLabel = _UserItem.roleLabelLocalized(
                    ...
                    final card = DecoratedBox(
                      ...
                      child: ListTile(
                        ...
                        leading: CircleAvatar(
                          radius: 18,
                          backgroundColor:
                              (locked ? AppColors.error : AppColors.primary)
  ...
  Future<void> _toggleActive(_UserItem u, bool isActive) async {
    ...
    if (!isActive && self) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(t.userManageCannotLockSelf)));
      return;
    }
    try {
      await FirebaseFirestore.instance.collection('users').doc(u.id).update({
      ...
      await AuditLogService.userActiveToggled(
        targetUserId: u.id,
        isActive: isActive,
      );
    } catch (e) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(t.cannotUpdateStatus('$e'))));
    }
  }
}
